...
const PREFIXES = [
  ...
  'ХИТ', 'АКТУАЛЬНО', 'ПРЕМИУМ', '100% РАБОЧИЙ', 'БЫСТРО', 'ОНЛАЙН'
];
...
const TRUST_TAGS = [
  '[ГАРАНТИЯ 100%]', '[БЫСТРАЯ ВЫДАЧА]', '[БЕЗ БАНА]', '[СВЕЖИЙ ПАТЧ]', 
  '[ОТ ПРО-ИГРОКА]', '[ЛИЧНО В РУКИ]', '[ПОДДЕРЖКА 24/7]', '[ПРОВЕРЕНО]', '[БЕЗ ПОСРЕДНИКОВ]'
];
...
export class ContentGenerator {
  ...
  async generateDescription(lot, options = {}) {
    ...
    const coreDesc = (lot.shortDesc || lot.description || '')
      ...
      .trim()
      .substring(0, 250);
    const safety = `✅ 100% безопасность для аккаунта. Быстрая личная передача товара в чате.`;
    const support = `💬 Продавец онлайн. Отвечу на любые вопросы и подробно проконсультирую!`;
